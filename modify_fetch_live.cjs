...
const targetFunctionStart = `const fetchLivePrice = async (asset: string) => {`;
const startIdx = content.indexOf(targetFunctionStart);

if (startIdx !== -1) {
    console.log("Replacing fetchLivePrice");
    ...
    const endIndexRegex = /const combinedData = \{\s*\.\.\.entryData,[\s\S]*?setLivePrice\(combinedData\);\s*return combinedData;\s*\}\s*catch\s*\(err:\s*any\)\s*\{\s*console\.error\([^\)]*\);\s*throw\s*err;\s*\}\s*finally\s*\{\s*setIsFetchingPrice\(false\);\s*\}\s*\};\s*/;
    
    // manual string replace logic because regex over multi-line is tricky in js string methods without m flag, and safer to just do substring replacements.
}
