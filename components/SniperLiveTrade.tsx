...
import { TradingStyle, SignalData, UserMetadata, Trade } from '../types';
import { Loader } from './Loader';
import { fetchMarketData } from '../services/twelveDataService';
import { saveAnalysis, updateTradeOutcome } from '../services/historyService';
import { performAutoLearning } from '../services/learningService';
...
export const SniperLiveTrade: React.FC<SniperLiveTradeProps> = ({ onBack, userMetadata, isLocked }) => {
  ...
  const handleOptimization = async () => {
    ...
    try {
      const result = await performAutoLearning(true);
      if (result) {
  ...
  const handleSetOutcome = async (msgId: string, tradeId: string, outcome: 'Win' | 'Loss' | 'No Trade') => {
    try {
      await updateTradeOutcome(tradeId, outcome);
      
      // Update local state
      setMessages(prev => prev.map(m => {
        if (m.id === msgId && m.signal) {
          return {
            ...m,
            signal: {
              ...m.signal,
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-800 dark:text-slate-200 font-sans selection:bg-emerald-500/30 transition-colors duration-300">
      ...
      {/* Neural Core Optimization Indicator */}
      <AnimatePresence>
        {(isOptimizing || lastOptimization) && (
          <motion.div 
            ...
            <div className="max-w-4xl mx-auto px-4 py-2 flex items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">
                  {isOptimizing ? 'Neural Network Retraining in Progress...' : 'Neural Adaptation Synchronized'}
                </span>
              </div>
              {lastOptimization && !isOptimizing && (
                <div className="text-[9px] font-bold opacity-90 truncate max-w-md">
                  New Rule: {lastOptimization}
                </div>
              )}
              {!isOptimizing && (
                <button 
            ...
            <div className="mb-8 sticky top-20 z-40 bg-slate-50/80 dark:bg-[#020617]/80 backdrop-blur-md py-2 transition-colors duration-300">
              ...
              <div className="flex items-center justify-end mb-4 px-1">
                <button 
                  ...
                  className="text-[10px] font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 px-4 py-2 rounded-xl border border-emerald-500/20 hover:bg-emerald-500/20 transition-all flex items-center gap-2 disabled:opacity-50"
                >
                            ...
                            <div className={`relative overflow-hidden rounded-[2.5rem] border p-8 backdrop-blur-xl ${
                              ...
                              <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                                ...
                                <div className="flex flex-col md:flex-row items-end gap-6">
                                  {msg.signal.comebackTimeMinutes && (
                                    <div className="flex flex-col items-end">
                                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-500 mb-1">Sniper Proj.</span>
                                      <div className="flex items-center gap-2 bg-amber-500/5 border border-amber-500/20 px-3 py-1.5 rounded-xl">
                                        <Clock className="w-3.5 h-3.5 text-amber-500 animate-pulse" />
                                        <div className="text-sm font-black tracking-tight text-amber-600 dark:text-amber-400 uppercase">
                                          {msg.signal.comebackTimeMinutes}M Wait
                                        </div>
                                      </div>
                                    </div>
                                  )}
                                  {msg.signal.signal !== 'NEUTRAL' && (
                                    <div className="flex flex-col items-end">
                                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-1">Feedback Loop</span>
                                      <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl border border-slate-200 dark:border-slate-800/50">
                                        <button 
                                          onClick={() => handleSetOutcome(msg.id, msg.signal!.id, 'Win')}
                                          className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1.5 ${
                                            msg.signal.outcome === 'Win' 
                                              ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' 
                                              : 'hover:bg-emerald-500/10 text-emerald-600 dark:text-emerald-500'
                                          }`}
                                        >
                                          <Trophy className="w-3 h-3" /> Win
                                        </button>
                                        <button 
                                          onClick={() => handleSetOutcome(msg.id, msg.signal!.id, 'Loss')}
                                          className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1.5 ${
                                            msg.signal.outcome === 'Loss' 
                                              ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/20' 
                                              : 'hover:bg-rose-500/10 text-rose-600 dark:text-rose-500'
                                          }`}
                                        >
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div className="w-12 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                                        <motion.div 
                                          ...
                                          className={`h-full rounded-full ${msg.signal.signal === 'BUY' ? 'bg-emerald-500' : msg.signal.signal === 'SELL' ? 'bg-rose-500' : 'bg-slate-500'}`}
                                        />
                                      </div>
                                    </div>
                                  </div>
